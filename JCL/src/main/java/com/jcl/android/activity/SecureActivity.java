		...
		getSupportFragmentManager()
				...
				.replace(
						...
						WebViewFragment
								.newInstance(
										//C.BASE_URL
										C.BASE_URL+"/secure?userid="+JCLApplication.getInstance().getUserId()))
				.commit();
	}
		...
		btnRightText.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				.replace(
						...
						WebViewFragment
								.newInstance(
										//C.BASE_URL http://192.168.1.101:8081/hsdata
										C.BASE_URL+"/secure/baoxiandetail?operate=A&userid="+JCLApplication.getInstance().getUserId()))
				.commit();
			}
		});
	}
